...
from src.candidates import iter_candidate_urls, iter_candidate_urls_with_generic_sites
from src.downloader import load_config_file
...
class TestCore(unittest.TestCase):
    def test_title_match_score(self):
        ...
        s1 = title_match_score(a, b)
        s2 = title_match_score(b, a)
    ...
    def test_generic_site_templates_expand(self):
        item = ReferenceItem(number=1, text="A sample paper title", dois=["10.1000/xyz"], urls=[])
        sites = [
            "https://sci-hub.se/{doi}",
            "https://example.org/search?q={title_encoded}",
        ]
        c = list(iter_candidate_urls_with_generic_sites(item, use_doi=True, generic_download_sites=sites))
